...
HEIGHT = 480
RUN_PIN = 44
LOCAL_ONLY_SCREEN = True
DOOR_FEATURE_ENABLED = False
DEBOUNCE_MS = 50
CYCLE_END_OFF_MS = 1000
MIN_CYCLE_SECONDS = 5
...
STATS_UPLOAD_INTERVAL_MS = 300000
...
PPH_HISTORY_STORAGE_FILE = "machine_oee_pph_history.json"
...
def make_nav_button(parent, text, x, y):
    btn = lv.btn(parent)
    btn.set_width(76)
    btn.set_height(35)
    btn.set_x(x)
    ...
    stabilize_button(btn)
    btn.set_style_radius(0, lv.PART.MAIN | lv.STATE.DEFAULT)
    btn.set_style_bg_color(lv.color_hex(0x444242), lv.PART.MAIN | lv.STATE.DEFAULT)
    btn.set_style_bg_opa(255, lv.PART.MAIN | lv.STATE.DEFAULT)
    btn.set_style_border_width(0, lv.PART.MAIN | lv.STATE.DEFAULT)
    lbl = lv.label(btn)
    lbl.set_text(text)
    lbl.center()
    lbl.set_style_text_color(lv.color_hex(0xE8E8E8), lv.PART.MAIN | lv.STATE.DEFAULT)
    return btn, lbl
...
settings_menu_hint.set_text("MAIN returns")
...
ui_SETTINGS_MAIN_BUTTON, ui_SETTINGS_MAIN_LABEL = make_nav_button(ui_SETTINGS_MENU, "MAIN", -360, -222)
...
pph_history_samples = []
pph_history_upload_dirty_days = {}
pph_history_storage_dirty = False
...
def apply_light_screen_mode_ui():
    if not LOCAL_ONLY_SCREEN:
        return
    set_object_hidden(ui_NOTE_BUTTON, True)
    set_object_hidden(ui_NOTE_LABEL, True)
...
def get_stats_history_dir_path():
    return "machines/{}/pph_history".format(stats_machine_id)


def get_stats_history_day_path(day_key):
    return "{}/{}.json".format(get_stats_history_dir_path(), str(day_key).strip())
...
def get_local_date_key_for_epoch(epoch):
    return format_date_key(time.localtime(epoch))
    ...
    for entry in entries:
        ...
        try:
            if isinstance(entry, dict):
                ...
                sample_epoch = int(float(entry[0]))
                current_pph = max(0, int(entry[1]))
            else:
                continue
        except Exception:
            continue
...
def build_local_pph_history_payload():
    rows = []
    for entry in pph_history_samples:
        try:
            ...
            continue
        if sample_epoch > 0:
            rows.append([sample_epoch, current_pph])
    return {"samples": rows}


def load_local_pph_history():
    if LOCAL_ONLY_SCREEN:
        return []
    if os is None or json is None:
        return None
    try:
        with open(PPH_HISTORY_STORAGE_FILE, "r") as f:
            payload = json.load(f)
    except Exception:
        return None
    if isinstance(payload, dict):
        payload = payload.get("samples", [])
    return normalize_pph_history_state(payload if isinstance(payload, list) else [])


def save_local_pph_history(force=False):
    global pph_history_storage_dirty
    if LOCAL_ONLY_SCREEN:
        pph_history_storage_dirty = False
        return
    if os is None or json is None:
        return
    if not force and not pph_history_storage_dirty:
        return
    try:
        with open(PPH_HISTORY_STORAGE_FILE, "w") as f:
            f.write(json.dumps(build_local_pph_history_payload()))
        pph_history_storage_dirty = False
    except Exception as err:
        print("PPH history save failed:", err)
...
def prune_pph_history_samples(now_epoch=None):
    ...
    for entry in pph_history_samples:
        ...
        if sample_epoch >= cutoff_epoch and sample_epoch > 0:
            ...
            removed_day_keys[get_local_date_key_for_epoch(sample_epoch)] = True
    changed = kept != pph_history_samples
    pph_history_samples = kept
    return changed, list(removed_day_keys.keys())
...
def mark_pph_history_day_dirty(day_key):
    global pph_history_upload_dirty
    day_key = str(day_key or "").strip()
    if not day_key:
        return
    pph_history_upload_dirty_days[day_key] = True
...
def record_pph_history_sample(now_epoch=None):
    ...
    if pph_history_samples and int(pph_history_samples[-1]["epoch"]) == bucket_epoch:
        return
    changed, removed_day_keys = prune_pph_history_samples(now_epoch)
    for day_key in removed_day_keys:
        mark_pph_history_day_dirty(day_key)
    current_pph = get_current_machine_pph(now_epoch)
    if not pph_history_samples or int(pph_history_samples[-1]["epoch"]) != bucket_epoch:
        pph_history_samples.append({"epoch": bucket_epoch, "pph": current_pph})
        changed = True
        mark_pph_history_day_dirty(get_local_date_key_for_epoch(bucket_epoch))
    if changed:
        pph_history_storage_dirty = True
        save_local_pph_history(force=True)
...
def build_pph_history_payload(day_key):
    points = []
    for entry in pph_history_samples:
        try:
            sample_epoch = int(float(entry.get("epoch", 0)))
        ...
        if sample_epoch <= 0:
            continue
        if get_local_date_key_for_epoch(sample_epoch) != day_key:
            continue
        dt_tuple = time.localtime(sample_epoch)
        minute_of_day = int(dt_tuple[3] * 60 + dt_tuple[4])
        points.append([minute_of_day, current_pph])
...
def write_stats_repo_file(api_path, payload, commit_message, existing_sha=None):
    payload_text = json_pretty_text(payload)
    commit_body = {
        "message": commit_message,
        "content": base64_encode_bytes(payload_text.encode("utf-8")),
...
def write_stats_repo_file_compact(api_path, payload, commit_message, existing_sha=None):
...
def upload_stats_to_github(manual=False):
    global stats_upload_dirty, pph_history_upload_dirty, last_stats_upload_ms, pph_history_upload_dirty_days
    if LOCAL_ONLY_SCREEN:
        raise OSError("Remote stats disabled")
    ...
    if pph_history_upload_dirty or manual:
        day_keys = []
        if manual and not pph_history_upload_dirty_days:
            for entry in pph_history_samples:
                try:
                    sample_epoch = int(float(entry.get("epoch", 0)))
                except Exception:
                    continue
                if sample_epoch > 0:
                    pph_history_upload_dirty_days[get_local_date_key_for_epoch(sample_epoch)] = True
        for day_key in pph_history_upload_dirty_days.keys():
            if day_key not in day_keys:
                day_keys.append(day_key)
        day_keys.sort()
        for day_key in day_keys:
            history_api_path, _existing_history, existing_history_sha = read_stats_repo_file(get_stats_history_day_path(day_key))
            history_payload = build_pph_history_payload(day_key)
            write_stats_repo_file_compact(
                history_api_path,
                history_payload,
                "Update {} PPH history {} from {}".format(stats_machine_id, day_key, APP_VERSION),
                existing_history_sha,
            )
            try:
                del pph_history_upload_dirty_days[day_key]
            except Exception:
                pass
        pph_history_upload_dirty = False
...
def load_state():
    ...
    storage_history_samples = load_local_pph_history()
    ...
    if state is None:
        if storage_history_samples is not None:
            pph_history_samples = storage_history_samples
        return
    ...
    if prune_pph_history_samples(time.time())[0]:
        pph_history_storage_dirty = True
    data_dirty = False
    last_save_ms = time.ticks_ms()
    return
...
def ensure_shift_period_reset():
    ...
    for shift_name in SHIFT_NAMES:
        ...
        if not stored_shift_key:
            ...
            changed = True
            continue
        if stored_shift_key != current_shift_key:
            capture_completed_shift_snapshot(shift_name, stored_shift_key, now_epoch)
            reset_graph_shift_data(shift_name)
            reset_pending_shift_data(shift_name, update_key=False)
            shift_reset_keys[shift_name] = current_shift_key
            changed = True
...
def ensure_software_update_popup():
    ...
    if LOCAL_ONLY_SCREEN:
        software_update_stats.add_flag(lv.obj.FLAG.HIDDEN)
...
def software_update_stats_event(e):
    if e.get_code() != lv.EVENT.CLICKED:
        return
    if LOCAL_ONLY_SCREEN:
        software_update_status.set_text("Remote stats disabled on ESP32 build")
        return
...
def apply_stats_config_from_popup():
    global stats_machine_id, stats_token_import_url, github_stats_token
    global rolling_pph_samples, pph_history_samples, pph_history_upload_dirty, pph_history_upload_dirty_days
    global pph_history_storage_dirty
...
def stats_config_upload_event(e):
    if e.get_code() != lv.EVENT.CLICKED:
        return
    if LOCAL_ONLY_SCREEN:
        stats_config_status.set_text("Remote stats disabled on ESP32 build")
        return
    try:
        apply_stats_config_from_popup()
...
ui_STATS_MAIN_BUTTON.add_event_cb(main_nav_button_event, lv.EVENT.ALL, None)
ui_SETTINGS_MAIN_BUTTON.add_event_cb(main_nav_button_event, lv.EVENT.ALL, None)
...
load_config()
ensure_config_file()
save_local_pph_history()
apply_signal_mode()
set_door_switch_visibility()
apply_light_screen_mode_ui()
    ...
    try:
        if startup_splash_active:
            ...
            if raw != last_raw:
                ...
                confirm_ms = CYCLE_END_OFF_MS if (machine_high and not signal_active) else DEBOUNCE_MS
                if time.ticks_diff(now_ms, last_change_ms) < confirm_ms:
                    sync_live_timers()
