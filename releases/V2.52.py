...
UI_REFRESH_ACTIVE_MS = 500
UI_REFRESH_IDLE_MS = 2000
UI_REFRESH_SETTINGS_MS = 2000
...
SUPERVISOR_PPH_WINDOW_SECONDS = 300
PPH_HISTORY_SAMPLE_SECONDS = 300
PPH_HISTORY_KEEP_DAYS = 14
PPH_HISTORY_KEEP_SECONDS = PPH_HISTORY_KEEP_DAYS * 86400
...
ui_SHIFT_DATA_BUTTON = lv.btn(ui_MAIN_SCREEN)
ui_SHIFT_DATA_BUTTON.set_width(60)
ui_SHIFT_DATA_BUTTON.set_height(35)
ui_SHIFT_DATA_BUTTON.set_x(-304)
ui_SHIFT_DATA_BUTTON.set_y(-222)
ui_SHIFT_DATA_BUTTON.set_align(lv.ALIGN.CENTER)
SetFlag(ui_SHIFT_DATA_BUTTON, lv.obj.FLAG.SCROLLABLE, False)
stabilize_button(ui_SHIFT_DATA_BUTTON)
ui_SHIFT_DATA_BUTTON.set_style_radius(0, lv.PART.MAIN | lv.STATE.DEFAULT)
ui_SHIFT_DATA_BUTTON.set_style_bg_color(lv.color_hex(0x444242), lv.PART.MAIN | lv.STATE.DEFAULT)
ui_SHIFT_DATA_BUTTON.set_style_bg_opa(255, lv.PART.MAIN | lv.STATE.DEFAULT)
ui_SHIFT_DATA_BUTTON.set_style_border_width(0, lv.PART.MAIN | lv.STATE.DEFAULT)

ui_SHIFT_DATA_LABEL = lv.label(ui_MAIN_SCREEN)
ui_SHIFT_DATA_LABEL.set_text("SHIFT")
ui_SHIFT_DATA_LABEL.set_x(-304)
ui_SHIFT_DATA_LABEL.set_y(-223)
ui_SHIFT_DATA_LABEL.set_align(lv.ALIGN.CENTER)
ui_SHIFT_DATA_LABEL.set_style_text_color(lv.color_hex(0xCFCFCF), lv.PART.MAIN | lv.STATE.DEFAULT)
ui_SHIFT_DATA_LABEL.set_style_text_font(safe_font("font_montserrat_14"), lv.PART.MAIN | lv.STATE.DEFAULT)
...
ui_Door_Switch_Invert_Button, ui_Door_Switch_Invert_Text = make_settings_button(ui_SETTINGS_MENU, "Door Invert\nOFF", 303, 183)
...
invert_save = None
invert_cancel = None
door_invert_popup = None
door_invert_checkbox = None
door_invert_save = None
door_invert_cancel = None
...
current_cycle_load_seconds = 0.0
current_cycle_idle_seconds = 0.0
last_cycle_aux_sync_epoch = None
door_window_shift = None
door_window_last_sync_ms = None
door_window_load_seconds = 0.0
door_window_idle_seconds = 0.0
...
rolling_pph_samples = {shift_name: [] for shift_name in SHIFT_NAMES}
pph_history_samples = []
...
pph_history_upload_dirty = False
...
shift_reset_epochs = {shift_name: 0.0 for shift_name in SHIFT_NAMES}
...
def iso_local_from_epoch(epoch_value):
    epoch_value = int(max(0, epoch_value))
    dt_tuple = time.localtime(epoch_value)
    return "{:04d}-{:02d}-{:02d}T{:02d}:{:02d}:{:02d}".format(
        dt_tuple[0], dt_tuple[1], dt_tuple[2], dt_tuple[3], dt_tuple[4], dt_tuple[5]
    )
...
def is_door_signal_active(raw_value):
    active = raw_value == 0
    if door_io_invert:
        active = not active
    return active
...
def set_door_switch_visibility():
    hidden = (not DOOR_FEATURE_ENABLED) or (not door_switch_enabled)
    for obj in (
        ui_Load_Time_Lable,
    ...
    if not DOOR_FEATURE_ENABLED:
        for obj in (
            ui_Door_Switch_Enable_Button,
            ui_Door_Switch_Enable_Lable,
            ui_Door_Switch_Invert_Button,
            ui_Door_Switch_Invert_Text,
        ):
            set_object_hidden(obj, True)


def reset_current_cycle_aux_metrics(anchor_epoch=None):
    global current_cycle_load_seconds, current_cycle_idle_seconds, last_cycle_aux_sync_epoch
    current_cycle_load_seconds = 0.0
    current_cycle_idle_seconds = 0.0
    last_cycle_aux_sync_epoch = anchor_epoch


def clear_door_window():
    global door_window_shift, door_window_last_sync_ms, door_window_load_seconds, door_window_idle_seconds
    door_window_shift = None
    door_window_last_sync_ms = None
    door_window_load_seconds = 0.0
    door_window_idle_seconds = 0.0
...
def start_door_window(shift_name, anchor_epoch=None):
    global door_window_shift, door_window_last_sync_ms, door_window_load_seconds, door_window_idle_seconds
    if shift_name not in pending_shift_stats:
        clear_door_window()
        return
    door_window_shift = shift_name
    door_window_last_sync_ms = time.ticks_ms()
    door_window_load_seconds = 0.0
    door_window_idle_seconds = 0.0


def sync_door_window(now_epoch=None):
    global door_window_last_sync_ms, door_window_load_seconds, door_window_idle_seconds
    if door_window_shift not in pending_shift_stats:
        clear_door_window()
        return
    if not door_switch_enabled:
        return
    now_ms = time.ticks_ms()
    if machine_high:
        door_window_last_sync_ms = now_ms
        return
    current_shift = get_shift()
    if current_shift != door_window_shift:
        return
    if door_window_last_sync_ms is None:
        door_window_last_sync_ms = now_ms
        return
    elapsed = max(0.0, float(time.ticks_diff(now_ms, door_window_last_sync_ms)) / 1000.0)
    door_window_last_sync_ms = now_ms
    if elapsed <= 0.0:
        return
    if is_door_signal_active(door_pin.value()):
        door_window_load_seconds += elapsed
    else:
        door_window_idle_seconds += elapsed


def commit_door_window():
    shift_name = door_window_shift
    if shift_name not in pending_shift_stats:
        clear_door_window()
        return
    load_seconds = max(0.0, float(door_window_load_seconds))
    idle_seconds = max(0.0, float(door_window_idle_seconds))
    if load_seconds > 0.0 or idle_seconds > 0.0:
        pending_shift_stats[shift_name]["load_sum"] += load_seconds
        pending_shift_stats[shift_name]["idle_sum"] += idle_seconds
        pending_shift_stats[shift_name]["door_cycle_count"] += 1
        graph_shift_stats[shift_name]["load_sum"] += load_seconds
        graph_shift_stats[shift_name]["idle_sum"] += idle_seconds
        graph_shift_stats[shift_name]["door_cycle_count"] += 1
        mark_data_dirty()
    clear_door_window()


def sync_current_cycle_aux_metrics(now_epoch=None):
    global last_cycle_aux_sync_epoch, current_cycle_load_seconds, current_cycle_idle_seconds
    if current_cycle_start_epoch is None or current_cycle_shift not in pending_shift_stats:
        last_cycle_aux_sync_epoch = None
        return
    ...
    if last_cycle_aux_sync_epoch is None:
        last_cycle_aux_sync_epoch = now_epoch
        return

    elapsed = max(0.0, float(now_epoch - last_cycle_aux_sync_epoch))
    last_cycle_aux_sync_epoch = now_epoch
    if elapsed <= 0.0 or not door_switch_enabled or machine_high:
        return

    if is_door_signal_active(door_pin.value()):
        current_cycle_load_seconds += elapsed
    else:
        current_cycle_idle_seconds += elapsed
...
def get_stats_history_path():
    return "machines/{}/pph_history.json".format(stats_machine_id)
...
def get_local_midnight_epoch(epoch):
    now = time.localtime(epoch)
    return time.mktime((now[0], now[1], now[2], 0, 0, 0, 0, 0))


def get_local_date_key_for_epoch(epoch, day_delta=0):
    target_epoch = get_local_midnight_epoch(epoch) + (int(day_delta) * 86400)
    return format_date_key(time.localtime(target_epoch))
...
def get_daily_production_key_for_epoch(epoch):
    ...
    if minutes < DAILY_PRODUCTION_RESET_MINUTE:
        return get_local_date_key_for_epoch(epoch, -1)
...
def get_shift_reset_info(shift_name, epoch=None):
    if epoch is None:
        epoch = time.time()

    config = shift_settings[shift_name]
    midnight_epoch = get_local_midnight_epoch(epoch)
    latest_key = get_local_date_key_for_epoch(epoch, -1)
    latest_reset_epoch = (midnight_epoch - 86400) + (config["start"] * 60) - (shift_reset_lead_minutes * 60)
    ...
    return latest_key, latest_reset_epoch


def get_shift_reset_key(shift_name, epoch=None):
    latest_key, _ = get_shift_reset_info(shift_name, epoch)
    return latest_key
...
def should_include_cycle_in_pph(shift_name, cycle_number, completion_epoch):
    ...
    shift_key, _ = get_shift_reset_info(shift_name, completion_epoch)
...
def clone_shift_bucket(bucket):
    return {
        ...
        "load_sum": float(bucket.get("load_sum", 0.0)),
        "idle_sum": float(bucket.get("idle_sum", 0.0)),
        "door_cycle_count": int(bucket.get("door_cycle_count", bucket.get("with_load_count", 0))),
...
def shift_bucket_has_activity(bucket):
    return (
        int(bucket.get("good", 0)) > 0
        or int(bucket.get("bad", 0)) > 0
        or int(bucket.get("cycle_count", 0)) > 0
        or int(bucket.get("with_load_count", 0)) > 0
        or int(bucket.get("door_cycle_count", 0)) > 0
        or int(bucket.get("pph_cycle_count", 0)) > 0
        or float(bucket.get("with_load_sum", 0.0)) > 0.0
        or float(bucket.get("load_sum", 0.0)) > 0.0
        or float(bucket.get("idle_sum", 0.0)) > 0.0
        or float(bucket.get("pph_with_load_sum", 0.0)) > 0.0
    )


def shift_name_has_saved_activity(shift_name):
    return (
        shift_bucket_has_activity(graph_shift_stats[shift_name])
        or shift_bucket_has_activity(pending_shift_stats[shift_name])
        or shift_bucket_has_activity(completed_shift_stats[shift_name])
    )


def prime_loaded_shift_markers(loaded_state_path, now_epoch=None):
    if now_epoch is None:
        now_epoch = time.time()

    loaded_state_path = str(loaded_state_path or "").strip()
    current_state_path = get_state_file_path()
    if loaded_state_path != current_state_path:
        return

    for shift_name in SHIFT_NAMES:
        if not shift_name_has_saved_activity(shift_name):
            continue
        if not shift_reset_keys[shift_name]:
            shift_reset_keys[shift_name] = get_shift_reset_key(shift_name, now_epoch)
        if float(shift_reset_epochs.get(shift_name, 0.0)) <= 0.0:
            _shift_key, shift_reset_epoch = get_shift_reset_info(shift_name, now_epoch)
            shift_reset_epochs[shift_name] = float(shift_reset_epoch)
...
def get_bucket_average_load(bucket):
    if int(bucket.get("door_cycle_count", 0)) <= 0:
        return 0
    return float(bucket.get("load_sum", 0.0)) / int(bucket.get("door_cycle_count", 0))


def get_bucket_average_idle(bucket):
    if int(bucket.get("door_cycle_count", 0)) <= 0:
        return 0
    return float(bucket.get("idle_sum", 0.0)) / int(bucket.get("door_cycle_count", 0))


def prune_rolling_pph_samples(now_epoch=None, notify_change=False):
    if now_epoch is None:
        now_epoch = time.time()
    cutoff_epoch = float(now_epoch) - SUPERVISOR_PPH_WINDOW_SECONDS
    changed = False
    for shift_name in SHIFT_NAMES:
        current_entries = rolling_pph_samples.get(shift_name, [])
        kept = []
        for sample in current_entries:
            try:
                sample_epoch = float(sample.get("epoch", 0.0))
                produced = max(0, int(sample.get("parts", 0)))
            except Exception:
                continue
            if sample_epoch >= cutoff_epoch and produced > 0:
                kept.append({"epoch": sample_epoch, "parts": produced})
        if kept != current_entries:
            changed = True
        rolling_pph_samples[shift_name] = kept
    if changed and notify_change:
        mark_data_dirty()
    return changed


def append_rolling_pph_sample(shift_name, produced_parts, completion_epoch):
    if shift_name not in rolling_pph_samples:
        return
    produced_parts = max(0, int(produced_parts))
    if produced_parts <= 0:
        return
    rolling_pph_samples[shift_name].append(
        {
            "epoch": float(completion_epoch),
            "parts": produced_parts,
        }
    )
    prune_rolling_pph_samples(completion_epoch, notify_change=False)
    mark_data_dirty()


def get_rolling_shift_pph(shift_name, now_epoch=None):
    if now_epoch is None:
        now_epoch = time.time()
    prune_rolling_pph_samples(now_epoch, notify_change=False)
    total_parts = 0
    for sample in rolling_pph_samples.get(shift_name, []):
        total_parts += max(0, int(sample.get("parts", 0)))
    if total_parts <= 0:
        return 0
    return int(round(total_parts * (3600.0 / float(SUPERVISOR_PPH_WINDOW_SECONDS))))
...
def get_live_bucket_pph(bucket, extra_seconds=0.0):
    eligible_cycles = int(bucket.get("pph_cycle_count", 0))
    total_parts = int(bucket.get("pph_produced_parts", bucket.get("produced_parts", bucket.get("cycle_count", 0))))
    total_time = float(bucket.get("pph_with_load_sum", bucket.get("with_load_sum", 0.0))) + max(0.0, float(extra_seconds))
    if eligible_cycles < PPH_DISPLAY_READY_CYCLES or total_parts <= 0 or total_time <= 0:
        return 0
    return int(round(total_parts / (total_time / 3600.0)))


def get_live_graph_shift_pph(shift_name):
    extra_seconds = 0.0
    if current_cycle_shift == shift_name and current_cycle_start_epoch is not None:
        anchor_epoch = graph_cycle_anchor_epoch.get(shift_name)
        live_start_epoch = current_cycle_start_epoch
        if anchor_epoch is not None and anchor_epoch > live_start_epoch:
            live_start_epoch = anchor_epoch
        extra_seconds = max(0.0, time.time() - live_start_epoch)
    return get_live_bucket_pph(graph_shift_stats[shift_name], extra_seconds=extra_seconds)
...
def get_pending_shift_average_load(shift_name):
    return get_bucket_average_load(pending_shift_stats[shift_name])


def get_pending_shift_average_idle(shift_name):
    return get_bucket_average_idle(pending_shift_stats[shift_name])


def get_live_pending_shift_average_load(shift_name):
    bucket = pending_shift_stats[shift_name]
    base_total = float(bucket.get("load_sum", 0.0))
    base_count = int(bucket.get("door_cycle_count", 0))
    if not door_switch_enabled or door_window_shift != shift_name:
        return get_bucket_average_load(bucket)
    live_load = float(door_window_load_seconds)
    if not machine_high and door_window_last_sync_ms is not None and get_shift() == shift_name:
        live_elapsed = max(0.0, float(time.ticks_diff(time.ticks_ms(), door_window_last_sync_ms)) / 1000.0)
        if is_door_signal_active(door_pin.value()):
            live_load += live_elapsed

    if live_load <= 0.0:
        return get_bucket_average_load(bucket)
    return (base_total + live_load) / float(base_count + 1)


def get_live_pending_shift_average_idle(shift_name):
    bucket = pending_shift_stats[shift_name]
    base_total = float(bucket.get("idle_sum", 0.0))
    base_count = int(bucket.get("door_cycle_count", 0))
    if not door_switch_enabled or door_window_shift != shift_name:
        return get_bucket_average_idle(bucket)
    live_idle = float(door_window_idle_seconds)
    if not machine_high and door_window_last_sync_ms is not None and get_shift() == shift_name:
        live_elapsed = max(0.0, float(time.ticks_diff(time.ticks_ms(), door_window_last_sync_ms)) / 1000.0)
        if not is_door_signal_active(door_pin.value()):
            live_idle += live_elapsed

    if live_idle <= 0.0:
        return get_bucket_average_idle(bucket)
    return (base_total + live_idle) / float(base_count + 1)
...
def normalize_rolling_pph_state(state_value):
    normalized = {shift_name: [] for shift_name in SHIFT_NAMES}
    if not isinstance(state_value, dict):
        return normalized
    for shift_name in SHIFT_NAMES:
        entries = state_value.get(shift_name, [])
        ...
        for entry in entries:
            if not isinstance(entry, dict):
                continue
            try:
                sample_epoch = float(entry.get("epoch", 0.0))
                produced = max(0, int(entry.get("parts", 0)))
            except Exception:
                continue
            if sample_epoch > 0 and produced > 0:
                cleaned.append({"epoch": sample_epoch, "parts": produced})
        normalized[shift_name] = cleaned
    now_epoch = time.time()
    cutoff_epoch = float(now_epoch) - SUPERVISOR_PPH_WINDOW_SECONDS
    for shift_name in SHIFT_NAMES:
        normalized[shift_name] = [entry for entry in normalized[shift_name] if float(entry["epoch"]) >= cutoff_epoch]
    return normalized


def normalize_pph_history_state(entries):
    normalized = []
    if not isinstance(entries, list):
        return normalized
    now_epoch = float(time.time())
    cutoff_epoch = now_epoch - float(PPH_HISTORY_KEEP_SECONDS)
    for entry in entries:
        if not isinstance(entry, dict):
            continue
        try:
            sample_epoch = int(float(entry.get("epoch", 0)))
            current_pph = max(0, int(entry.get("pph", 0)))
        except Exception:
            continue
        if sample_epoch < cutoff_epoch or sample_epoch <= 0:
            continue
        normalized.append({"epoch": sample_epoch, "pph": current_pph})
    normalized.sort(key=lambda item: item["epoch"])
    deduped = []
    for entry in normalized:
        if deduped and deduped[-1]["epoch"] == entry["epoch"]:
            deduped[-1]["pph"] = entry["pph"]
        else:
            deduped.append(entry)
    return deduped


def prune_pph_history_samples(now_epoch=None):
    global pph_history_samples
    if now_epoch is None:
        now_epoch = time.time()
    cutoff_epoch = float(now_epoch) - float(PPH_HISTORY_KEEP_SECONDS)
    kept = []
    for entry in pph_history_samples:
        try:
            sample_epoch = int(float(entry.get("epoch", 0)))
            current_pph = max(0, int(entry.get("pph", 0)))
        except Exception:
            continue
        if sample_epoch >= cutoff_epoch and sample_epoch > 0:
            kept.append({"epoch": sample_epoch, "pph": current_pph})
    changed = kept != pph_history_samples
    pph_history_samples = kept
    return changed


def current_pph_history_bucket_epoch(now_epoch=None):
    if now_epoch is None:
        now_epoch = time.time()
    return int(float(now_epoch) // int(PPH_HISTORY_SAMPLE_SECONDS)) * int(PPH_HISTORY_SAMPLE_SECONDS)


def get_current_machine_pph(now_epoch=None):
    active_shift = get_shift()
    if active_shift not in SHIFT_NAMES:
        return 0
    return int(get_rolling_shift_pph(active_shift, now_epoch))


def record_pph_history_sample(now_epoch=None):
    global pph_history_upload_dirty
    if now_epoch is None:
        now_epoch = time.time()
    changed = prune_pph_history_samples(now_epoch)
    bucket_epoch = current_pph_history_bucket_epoch(now_epoch)
    current_pph = get_current_machine_pph(now_epoch)
    if pph_history_samples and int(pph_history_samples[-1]["epoch"]) == bucket_epoch:
        if int(pph_history_samples[-1].get("pph", 0)) != current_pph:
            pph_history_samples[-1]["pph"] = current_pph
            changed = True
    else:
        pph_history_samples.append({"epoch": bucket_epoch, "pph": current_pph})
        changed = True

    if changed:
        pph_history_upload_dirty = True
        mark_data_dirty()
...
def build_machine_status_payload(existing=None):
    ...
    for shift_name in SHIFT_NAMES:
        ...
        shifts_payload[shift_name] = {
            ...
            "current_pph": int(get_rolling_shift_pph(shift_name)),
    ...
    return {
        ...
        "totals": {
            "good": int(good_count),
            "bad": int(bad_count),
            "combined": int(good_count + bad_count),
            "daily_production": int(daily_production_count),
        },
        "shifts": shifts_payload,
    }


def build_pph_history_payload():
    points = []
    for entry in pph_history_samples:
        try:
            sample_epoch = int(float(entry.get("epoch", 0)))
        ...
        if sample_epoch <= 0:
            continue
        points.append([iso_local_from_epoch(sample_epoch), current_pph])

    return {
        "machine_id": stats_machine_id,
        "app_version": APP_VERSION,
        "interval_minutes": int(PPH_HISTORY_SAMPLE_SECONDS // 60),
        "keep_days": int(PPH_HISTORY_KEEP_DAYS),
        "points": points,
    }


def read_stats_repo_file(path_text):
    api_path = get_stats_repo_api_path(path_text)
    ...
    return api_path, existing, existing_sha


def write_stats_repo_file(api_path, payload, commit_message, existing_sha=None):
    commit_body = {
    ...
    status_code, _headers, response_text = github_api_request("PUT", api_path, github_stats_token, commit_body)
    if status_code not in (200, 201):
        raise OSError("GitHub write failed ({}): {}".format(status_code, parse_github_error_message(response_text)))


def upload_stats_to_github(manual=False):
    global stats_upload_dirty, pph_history_upload_dirty, last_stats_upload_ms
    ...
    if stats_upload_dirty or manual:
        api_path, existing, existing_sha = read_stats_repo_file(get_stats_status_path())
        payload = build_machine_status_payload(existing)
        write_stats_repo_file(
            api_path,
            payload,
            "Update {} status from {}".format(stats_machine_id, APP_VERSION),
            existing_sha,
        )
        stats_upload_dirty = False

    if pph_history_upload_dirty or manual:
        history_api_path, _existing_history, existing_history_sha = read_stats_repo_file(get_stats_history_path())
        history_payload = build_pph_history_payload()
        write_stats_repo_file(
            history_api_path,
            history_payload,
            "Update {} PPH history from {}".format(stats_machine_id, APP_VERSION),
            existing_history_sha,
        )
        pph_history_upload_dirty = False
...
def maybe_upload_stats():
    ...
    if (not stats_upload_dirty and not pph_history_upload_dirty) or not stats_machine_id or not github_stats_token:
        return
    now_ms = time.ticks_ms()
...
def load_config():
    global pph_goal, parts_per_cycle, io_invert, door_io_invert, door_switch_enabled, shift_reset_lock
    ...
    try:
        ...
        door_io_invert = bool(state.get("door_io_invert", door_io_invert))
        door_switch_enabled = bool(state.get("door_switch_enabled", door_switch_enabled)) if DOOR_FEATURE_ENABLED else False
...
def save_state(force=False):
    ...
    state = {
        ...
        "machine_run_start_epoch": machine_run_start_epoch,
        "current_cycle_start_epoch": current_cycle_start_epoch,
        "current_cycle_shift": current_cycle_shift,
        "last_cycle_complete_epoch": last_cycle_complete_epoch,
        "last_daily_reset_key": last_daily_reset_key,
        "shift_reset_keys": shift_reset_keys,
        "shift_reset_epochs": shift_reset_epochs,
        ...
        "rolling_pph_samples": rolling_pph_samples,
        "pph_history_samples": pph_history_samples,
    }
...
def load_state():
    global good_count, bad_count, pph_goal, parts_per_cycle, io_invert, door_io_invert
    ...
    global pending_machine_run_start_epoch, pending_cycle_shift, current_cycle_start_epoch, current_cycle_shift
    global graph_shift_stats, pending_shift_stats, completed_shift_stats, graph_cycle_anchor_epoch, rolling_pph_samples, pph_history_samples
    global data_dirty, last_save_ms, last_daily_reset_key, shift_reset_keys, shift_reset_epochs, completed_shift_keys, shift_settings
    global current_machine_run_seconds, current_cycle_with_load_seconds, last_cycle_complete_epoch
    ...
    try:
        ...
        loaded_shift_reset_epochs = state.get("shift_reset_epochs", {})
        ...
        for shift_name in SHIFT_NAMES:
            ...
            try:
                shift_reset_epochs[shift_name] = float(loaded_shift_reset_epochs.get(shift_name, shift_reset_epochs[shift_name]))
            except Exception:
                shift_reset_epochs[shift_name] = 0.0
        ...
        rolling_pph_samples = normalize_rolling_pph_state(state.get("rolling_pph_samples", {}))
        pph_history_samples = normalize_pph_history_state(state.get("pph_history_samples", []))
        for shift_name in SHIFT_NAMES:
        ...
        prime_loaded_shift_markers(loaded_state_path)
        try:
            last_cycle_complete_epoch = float(state.get("last_cycle_complete_epoch", last_cycle_complete_epoch or 0.0))
        except Exception:
            last_cycle_complete_epoch = None
        try:
            loaded_cycle_start_epoch = float(state.get("current_cycle_start_epoch", 0.0))
        except Exception:
            loaded_cycle_start_epoch = 0.0
        loaded_cycle_shift = state.get("current_cycle_shift", None)
        try:
            loaded_machine_run_start_epoch = float(state.get("machine_run_start_epoch", loaded_cycle_start_epoch or 0.0))
        except Exception:
            loaded_machine_run_start_epoch = 0.0
        raw_value = run_pin.value()
        machine_high = is_signal_active(raw_value)
        machine_run_confirmed = machine_high
        if machine_high and loaded_machine_run_start_epoch > 0.0:
            machine_run_start_epoch = loaded_machine_run_start_epoch
        else:
            machine_run_start_epoch = time.time() if machine_high else None
        pending_machine_run_start_epoch = None
        pending_cycle_shift = None
        if machine_high and loaded_cycle_start_epoch > 0.0 and loaded_cycle_shift in pending_shift_stats:
            current_cycle_start_epoch = loaded_cycle_start_epoch
            current_cycle_shift = loaded_cycle_shift
        else:
            current_cycle_start_epoch = None
            current_cycle_shift = None
        current_machine_run_seconds = max(0, int(time.time() - machine_run_start_epoch)) if machine_high and machine_run_start_epoch is not None else 0
        if current_cycle_start_epoch is not None and current_cycle_shift == get_shift():
            current_cycle_with_load_seconds = max(0, int(time.time() - current_cycle_start_epoch))
        else:
            current_cycle_with_load_seconds = 0
        reset_current_cycle_aux_metrics()
        if not DOOR_FEATURE_ENABLED:
            door_switch_enabled = False
            clear_door_window()
    except Exception:
...
def auto_complete_cycle(shift_name=None, include_pph=False, completion_epoch=None):
    ...
    if shift_name is not None:
        ...
        if include_pph:
            ...
            append_rolling_pph_sample(shift_name, produced, completion_epoch if completion_epoch is not None else time.time())
    mark_data_dirty()
...
def reset_graph_shift_data(shift_name):
    ...
    rolling_pph_samples[shift_name] = []
    mark_data_dirty()
...
def reset_pending_shift_data(shift_name, update_key=True):
    ...
    if update_key:
        shift_reset_key, shift_reset_epoch = get_shift_reset_info(shift_name)
        shift_reset_keys[shift_name] = shift_reset_key
        shift_reset_epochs[shift_name] = float(shift_reset_epoch)
    mark_data_dirty()
...
def capture_completed_shift_snapshot(shift_name, shift_key, now_epoch=None):
    if not shift_key:
        return False
    if now_epoch is None:
        now_epoch = time.time()
    if completed_shift_keys[shift_name] == shift_key:
        return False
...
def ensure_shift_period_reset():
    ...
    now_epoch = time.time()
    for shift_name in SHIFT_NAMES:
        current_shift_key, current_reset_epoch = get_shift_reset_info(shift_name, now_epoch)
        stored_shift_key = shift_reset_keys[shift_name]
        stored_reset_epoch = float(shift_reset_epochs.get(shift_name, 0.0))
        shift_has_activity = shift_name_has_saved_activity(shift_name)
        if not stored_shift_key:
            shift_reset_keys[shift_name] = current_shift_key
            if shift_has_activity and stored_reset_epoch <= 0.0:
                shift_reset_epochs[shift_name] = float(current_reset_epoch)
            changed = True
            stored_shift_key = shift_reset_keys[shift_name]
            stored_reset_epoch = float(shift_reset_epochs.get(shift_name, 0.0))
        graph_anchor_epoch = graph_cycle_anchor_epoch.get(shift_name)
        graph_stale = (
            graph_anchor_epoch is not None
            and float(graph_anchor_epoch) < float(current_reset_epoch)
            and shift_bucket_has_activity(graph_shift_stats[shift_name])
        )
        pending_stale = graph_stale and shift_bucket_has_activity(pending_shift_stats[shift_name])
        marker_stale = stored_reset_epoch > 0.0 and stored_reset_epoch < float(current_reset_epoch)

        if (stored_shift_key and stored_shift_key != current_shift_key) or marker_stale or graph_stale or pending_stale:
            archive_shift_key = stored_shift_key
            if pending_stale and archive_shift_key == current_shift_key:
                archive_shift_key = get_shift_reset_key(shift_name, current_reset_epoch - 1)
            if marker_stale and (not archive_shift_key or archive_shift_key == current_shift_key):
                archive_shift_key = get_shift_reset_key(shift_name, current_reset_epoch - 1)
            capture_completed_shift_snapshot(shift_name, archive_shift_key, now_epoch)
            reset_graph_shift_data(shift_name)
            reset_pending_shift_data(shift_name, update_key=False)
            shift_reset_keys[shift_name] = current_shift_key
            shift_reset_epochs[shift_name] = float(current_reset_epoch)
...
def finalize_completed_shifts():
    ...
    for shift_name in SHIFT_NAMES:
        ...
        if capture_completed_shift_snapshot(shift_name, shift_key, now_epoch):
            changed = True
...
def hide_door_invert_popup():
    if door_invert_popup is not None:
        door_invert_popup.add_flag(lv.obj.FLAG.HIDDEN)
...
def ensure_door_invert_popup():
    global door_invert_popup, door_invert_checkbox, door_invert_save, door_invert_cancel
    if door_invert_popup is not None:
        return

    door_invert_popup = lv.obj(ui_SETTINGS_MENU)
    door_invert_popup.set_size(340, 220)
    door_invert_popup.center()
    stabilize_widget(door_invert_popup)
    door_invert_popup.set_style_bg_color(lv.color_hex(0x222222), lv.PART.MAIN | lv.STATE.DEFAULT)
    door_invert_popup.set_style_bg_opa(255, lv.PART.MAIN | lv.STATE.DEFAULT)
    door_invert_popup.set_style_border_color(lv.color_hex(0x0A0ACC), lv.PART.MAIN | lv.STATE.DEFAULT)
    door_invert_popup.set_style_border_width(3, lv.PART.MAIN | lv.STATE.DEFAULT)
    door_invert_popup.set_style_radius(8, lv.PART.MAIN | lv.STATE.DEFAULT)
    door_invert_popup.clear_flag(lv.obj.FLAG.SCROLLABLE)
    door_invert_popup.add_flag(lv.obj.FLAG.HIDDEN)

    door_invert_title = lv.label(door_invert_popup)
    door_invert_title.set_text("DOOR SWITCH INVERT")
    door_invert_title.align(lv.ALIGN.TOP_MID, 0, 16)
    door_invert_title.set_style_text_color(lv.color_hex(0xFCA903), lv.PART.MAIN | lv.STATE.DEFAULT)
    door_invert_title.set_style_text_font(safe_font("font_montserrat_20"), lv.PART.MAIN | lv.STATE.DEFAULT)

    door_invert_checkbox = lv.checkbox(door_invert_popup)
    door_invert_checkbox.set_text("Invert IO43 active state")
    door_invert_checkbox.align(lv.ALIGN.CENTER, 0, -12)
    stabilize_widget(door_invert_checkbox)

    door_invert_save = make_button(door_invert_popup, "SAVE", -90, 72, 100, 42, 0x04BE2D)
    door_invert_cancel = make_button(door_invert_popup, "CANCEL", 90, 72, 100, 42, 0xC32331)
    door_invert_save.add_event_cb(door_invert_save_event, lv.EVENT.ALL, None)
    door_invert_cancel.add_event_cb(door_invert_cancel_event, lv.EVENT.ALL, None)
...
def ensure_io_check_popup():
    ...
    io_check_popup.set_size(380, 320)
    ...
    io_check_label.set_text("IO44 RAW = LOW\nIO43 RAW = LOW")
    io_check_label.align(lv.ALIGN.CENTER, 0, -18)
    ...
    io_check_label.set_style_text_align(lv.TEXT_ALIGN.CENTER, lv.PART.MAIN | lv.STATE.DEFAULT)

    io_check_close = make_button(io_check_popup, "CLOSE", 0, 118, 100, 40, 0xC32331)
...
def refresh_io_check_label():
    if io_check_label is None:
        return
    if not DOOR_FEATURE_ENABLED:
        io_check_label.set_text(
            "IO44 RAW = {}\nIO43 RAW = {}\nDoor Feature = OFF".format(
                "LOW" if run_pin.value() == 0 else "HIGH",
                "LOW" if door_pin.value() == 0 else "HIGH",
            )
        )
        return
    current_shift = get_shift()
    live_load = float(door_window_load_seconds)
    live_idle = float(door_window_idle_seconds)
    if door_window_last_sync_ms is not None and door_window_shift in pending_shift_stats and not machine_high and current_shift == door_window_shift:
        live_elapsed = max(0.0, float(time.ticks_diff(time.ticks_ms(), door_window_last_sync_ms)) / 1000.0)
        if is_door_signal_active(door_pin.value()):
            live_load += live_elapsed
        else:
    ...
    set_cached_label_text(
        ...
        "IO44 RAW = {}\nIO43 RAW = {}\nIO43 ACTIVE = {}\nDoor Inv = {}\nDoor Win = {}\nLoad Live = {}\nIdle Live = {}".format(
            "LOW" if run_pin.value() == 0 else "HIGH",
            "LOW" if door_pin.value() == 0 else "HIGH",
            "ON" if is_door_signal_active(door_pin.value()) else "OFF",
            "ON" if door_io_invert else "OFF",
            door_window_shift if door_window_shift is not None else "NONE",
            format_mmss(live_load),
            format_mmss(live_idle),
        ),
    )
...
def apply_signal_mode():
    ...
    pending_machine_run_start_epoch = None
    pending_cycle_shift = None
    reset_current_cycle_aux_metrics(time.time() if current_cycle_start_epoch is not None else None)
    if machine_high:
        clear_door_window()
    elif current_cycle_start_epoch is not None and current_cycle_shift in pending_shift_stats:
        start_door_window(current_cycle_shift, time.time())
    else:
        clear_door_window()
...
def confirm_pending_cycle_start():
    ...
    if current_cycle_start_epoch is None:
        ...
        reset_current_cycle_aux_metrics(now_epoch)
...
def apply_shift_settings_from_popup():
    ...
    for shift_name in SHIFT_NAMES:
        ...
        shift_reset_keys[shift_name], shift_reset_epoch = get_shift_reset_info(shift_name)
        shift_reset_epochs[shift_name] = float(shift_reset_epoch)
    current_cycle_start_epoch = None
    current_cycle_shift = None
    current_cycle_with_load_seconds = 0
...
def show_door_invert_popup():
    ensure_door_invert_popup()
    if door_io_invert:
        door_invert_checkbox.add_state(lv.STATE.CHECKED)
    else:
        door_invert_checkbox.clear_state(lv.STATE.CHECKED)
    door_invert_popup.clear_flag(lv.obj.FLAG.HIDDEN)
    door_invert_popup.move_foreground()
...
def apply_door_invert_setting():
    global door_io_invert
    door_io_invert = door_invert_checkbox.has_state(lv.STATE.CHECKED)
    reset_current_cycle_aux_metrics(time.time() if current_cycle_start_epoch is not None else None)
    if door_switch_enabled and not machine_high and current_cycle_shift in pending_shift_stats and current_cycle_start_epoch is not None:
        start_door_window(current_cycle_shift, time.time())
    else:
        clear_door_window()
    settings_menu_status.set_text("Door input invert {}".format("ON" if door_io_invert else "OFF"))
    mark_data_dirty()
    hide_door_invert_popup()
    update_ui()
    save_config()
...
def update_main_screen_ui(current_shift):
    ...
    if current_shift is None:
        ...
        if DOOR_FEATURE_ENABLED and door_switch_enabled:
            set_cached_label_text("avg_load_time", ui_AVRG_LOAD_TIME_NUMBER, format_mmss(get_live_pending_shift_average_load(current_shift)))
            set_cached_label_text("avg_idle_time", ui_AVRG_IDLE_TIME_NUMBER, format_mmss(get_live_pending_shift_average_idle(current_shift)))
        else:
            set_cached_label_text("avg_load_time", ui_AVRG_LOAD_TIME_NUMBER, "0m 0s")
            set_cached_label_text("avg_idle_time", ui_AVRG_IDLE_TIME_NUMBER, "0m 0s")
...
def update_settings_menu_ui(now_tuple):
    ...
    if DOOR_FEATURE_ENABLED:
        ...
        set_cached_label_text(
            "settings_door_invert_text",
            ui_Door_Switch_Invert_Text,
            "Door Invert\n{}".format("ON" if door_io_invert else "OFF"),
        )
    set_cached_label_text(
...
def sync_live_timers():
    ...
    now = time.time()
    prune_rolling_pph_samples(now, notify_change=True)
    record_pph_history_sample(now)
    active_shift = get_shift()
    if DOOR_FEATURE_ENABLED and (
        door_switch_enabled
        and not machine_high
        and active_shift is not None
        and current_cycle_start_epoch is not None
        and current_cycle_shift == active_shift
        and door_window_shift != active_shift
    ):
        start_door_window(active_shift, now)
    if DOOR_FEATURE_ENABLED:
        sync_current_cycle_aux_metrics(now)
        sync_door_window(now)
    else:
        reset_current_cycle_aux_metrics()
        clear_door_window()
...
def change_screen_gesture_event(e):
    ...
    if current_screen == ui_MAIN_SCREEN and (direction == lv.DIR.RIGHT or direction == lv.DIR.LEFT):
        hide_goal_popup()
        hide_count_popup()
...
def door_switch_toggle_event(e):
    if e.get_code() != lv.EVENT.CLICKED:
        return
    if not DOOR_FEATURE_ENABLED:
        settings_menu_status.set_text("Door feature disabled")
        return
...
def door_invert_save_event(e):
    if e.get_code() != lv.EVENT.CLICKED:
        return
    apply_door_invert_setting()


def door_invert_cancel_event(e):
    if e.get_code() != lv.EVENT.CLICKED:
        return
    hide_door_invert_popup()
...
def apply_stats_config_from_popup():
    global stats_machine_id, stats_token_import_url, github_stats_token
    global pph_history_samples, pph_history_upload_dirty
    previous_machine_id = stats_machine_id
    ...
    if previous_machine_id != stats_machine_id:
        pph_history_samples = []
        pph_history_upload_dirty = True
    ensure_machine_option_available(stats_machine_id)
    mark_data_dirty()
...
ui_SHIFT_DATA_BUTTON.add_event_cb(shift_data_button_event, lv.EVENT.ALL, None)
...
ui_Door_Switch_Invert_Button.add_event_cb(door_invert_event, lv.EVENT.ALL, None)
        ...
        if startup_splash_active:
            ...
            if raw != last_raw:
                ...
                if (not signal_active) and machine_high:
                    ...
                    sync_current_cycle_aux_metrics(now_epoch)
                    ...
                    if machine_run_start_epoch is not None and machine_run_confirmed:
                        ...
                        if current_cycle_start_epoch is not None and completed_cycle_shift in pending_shift_stats:
                            ...
                            auto_complete_cycle(completed_cycle_shift, include_pph=include_pph, completion_epoch=now_epoch)
                        elif current_cycle_start_epoch is not None:
                            auto_complete_cycle(None, completion_epoch=now_epoch)
                            current_cycle_with_load_seconds = 0
                        else:
                            current_cycle_with_load_seconds = 0
                        ...
                        if active_shift is not None and completed_cycle_shift == active_shift:
                            ...
                            reset_current_cycle_aux_metrics(now_epoch)
                            if DOOR_FEATURE_ENABLED and door_switch_enabled:
                                start_door_window(active_shift, now_epoch)
                            else:
                                clear_door_window()
                        else:
                            current_cycle_start_epoch = None
