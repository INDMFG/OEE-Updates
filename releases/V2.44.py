...
def get_live_pending_shift_average_load(shift_name):
    ...
    if (
        not door_switch_enabled
        or current_cycle_shift != shift_name
        or current_cycle_start_epoch is None
        or machine_high
    ):
        return get_bucket_average_load(bucket)

    live_load = float(current_cycle_load_seconds)
    if last_cycle_aux_sync_epoch is not None:
        live_elapsed = max(0.0, float(time.time() - last_cycle_aux_sync_epoch))
        if is_door_signal_active(door_pin.value()):
...
def get_live_pending_shift_average_idle(shift_name):
    ...
    live_idle = float(current_cycle_idle_seconds)
    if last_cycle_aux_sync_epoch is not None:
        live_elapsed = max(0.0, float(time.time() - last_cycle_aux_sync_epoch))
                    ...
                    if machine_run_start_epoch is not None and machine_run_confirmed:
                        ...
                        if current_cycle_start_epoch is not None and completed_cycle_shift in pending_shift_stats:
                            ...
                            if door_switch_enabled:
                                load_seconds = max(0.0, float(current_cycle_load_seconds))
                                idle_seconds = max(0.0, float(current_cycle_idle_seconds))
                                graph_shift_stats[completed_cycle_shift]["load_sum"] += load_seconds
                                graph_shift_stats[completed_cycle_shift]["idle_sum"] += idle_seconds
                                graph_shift_stats[completed_cycle_shift]["door_cycle_count"] += 1
                                pending_shift_stats[completed_cycle_shift]["load_sum"] += load_seconds
                                pending_shift_stats[completed_cycle_shift]["idle_sum"] += idle_seconds
                                pending_shift_stats[completed_cycle_shift]["door_cycle_count"] += 1
                            if include_pph:
