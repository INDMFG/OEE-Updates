...
def build_machine_status_payload(existing=None):
    ...
    shift_pph = {}
    shift_live_totals = {}
    shift_completed_totals = {}
    for shift_name in SHIFT_NAMES:
        shift_pph[shift_name] = int(get_graph_shift_pph(shift_name))
        shift_live_totals[shift_name] = {
        ...
        shift_completed_totals[shift_name] = {
            "good": int(completed_shift_stats[shift_name]["good"]),
            "bad": int(completed_shift_stats[shift_name]["bad"]),
            "cycles": int(completed_shift_stats[shift_name]["cycle_count"]),
        }

    now_tuple = time.localtime()
    current_shift = get_shift()
    return {
        "machine_name": existing.get("machine_name", stats_machine_id),
        ...
        "totals": {
            "good": int(good_count),
            "bad": int(bad_count),
            "daily_production": int(get_total_daily_good_parts()),
        },
        "pph": shift_pph,
        "shift_live_totals": shift_live_totals,
        "shift_completed_totals": shift_completed_totals,
...
def upload_stats_to_github(manual=False):
    ...
    if status_code == 200:
        ...
        raise OSError("GitHub read failed ({})".format(status_code))

    payload = build_machine_status_payload(existing)
    commit_body = {
        "message": "Update {} status from {}".format(stats_machine_id, APP_VERSION),
        "content": base64_encode_bytes(json.dumps(payload).encode("utf-8")),
    ...
    if status_code not in (200, 201):
        raise OSError("GitHub write failed ({})".format(status_code))
...
def maybe_upload_stats():
    ...
    try:
        ...
        settings_menu_status.set_text("Stats upload failed")
...
def load_state():
    ...
    global stats_machine_id, github_stats_token
...
def ensure_stats_config_popup():
    global stats_config_popup, stats_config_machine_textarea, stats_config_token_textarea
    global stats_config_status, stats_config_kb, stats_config_save, stats_config_upload, stats_config_close

    if stats_config_popup is not None:
        return

    stats_config_popup = lv.obj(ui_SETTINGS_MENU)
    stats_config_popup.set_size(640, 360)
    ...
    machine_label.align(lv.ALIGN.TOP_LEFT, 18, 82)
    ...
    stats_config_machine_textarea.align(lv.ALIGN.TOP_LEFT, 18, 108)
    ...
    token_label.align(lv.ALIGN.TOP_LEFT, 18, 156)
    ...
    stats_config_token_textarea.align(lv.ALIGN.TOP_LEFT, 18, 182)
    ...
    stats_config_status.align(lv.ALIGN.TOP_LEFT, 18, 228)
    ...
    stats_config_save = make_button(stats_config_popup, "SAVE", -180, -62, 110, 40, 0x04BE2D)
    stats_config_upload = make_button(stats_config_popup, "UPLOAD", 0, -62, 110, 40, 0x465AC4)
    stats_config_close = make_button(stats_config_popup, "CLOSE", 180, -62, 110, 40, 0xC32331)
...
def stats_config_upload_event(e):
    ...
    try:
        ...
        stats_config_status.set_text("Upload failed")
        settings_menu_status.set_text("Stats upload failed")
        print("Stats upload failed:", err)
