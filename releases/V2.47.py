...
door_window_last_sync_epoch = None
...
def clear_door_window():
    global door_window_shift, door_window_last_sync_epoch, door_window_load_seconds, door_window_idle_seconds
    door_window_shift = None
    door_window_last_sync_epoch = None
    door_window_load_seconds = 0.0
    door_window_idle_seconds = 0.0
...
def start_door_window(shift_name, anchor_epoch=None):
    ...
    door_window_last_sync_epoch = float(anchor_epoch)
    door_window_load_seconds = 0.0
    door_window_idle_seconds = 0.0


def sync_door_window(now_epoch=None):
    global door_window_last_sync_epoch, door_window_load_seconds, door_window_idle_seconds
    ...
    if now_epoch is None:
        now_epoch = time.time()
    if machine_high:
        door_window_last_sync_epoch = float(now_epoch)
        return
    current_shift = get_shift()
    if current_shift != door_window_shift:
        return
    if door_window_last_sync_epoch is None:
        door_window_last_sync_epoch = float(now_epoch)
        return
    elapsed = max(0.0, float(now_epoch) - float(door_window_last_sync_epoch))
    door_window_last_sync_epoch = float(now_epoch)
    if elapsed <= 0.0:
        return
...
def prune_rolling_pph_samples(now_epoch=None):
    if now_epoch is None:
        now_epoch = time.time()
...
def append_rolling_pph_sample(shift_name, produced_parts, completion_epoch):
    ...
    prune_rolling_pph_samples(completion_epoch)
    mark_data_dirty()
...
def get_rolling_shift_pph(shift_name, now_epoch=None):
    if now_epoch is None:
        now_epoch = time.time()
    prune_rolling_pph_samples(now_epoch)
    total_parts = 0
    for sample in rolling_pph_samples.get(shift_name, []):
...
def get_live_pending_shift_average_load(shift_name):
    ...
    if not machine_high and door_window_last_sync_epoch is not None and get_shift() == shift_name:
        live_elapsed = max(0.0, float(time.time() - door_window_last_sync_epoch))
        if is_door_signal_active(door_pin.value()):
...
def refresh_io_check_label():
    ...
    live_load = float(door_window_load_seconds)
    live_idle = float(door_window_idle_seconds)
    if door_window_last_sync_epoch is not None and door_window_shift in pending_shift_stats and not machine_high and current_shift == door_window_shift:
        live_elapsed = max(0.0, float(time.time() - door_window_last_sync_epoch))
        if is_door_signal_active(door_pin.value()):
            live_load += live_elapsed
        else:
